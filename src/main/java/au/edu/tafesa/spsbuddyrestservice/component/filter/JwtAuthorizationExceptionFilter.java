...
import au.edu.tafesa.spsbuddyrestservice.model.ErrorResponse;
...
public class JwtAuthorizationExceptionFilter extends GenericFilterBean {
    ...
    @Autowired
    public JwtAuthorizationExceptionFilter(ObjectMapper errorObjectMapper) {
        this.errorObjectMapper = errorObjectMapper;
    }
    ...
    private void sendErrorResponse(ServletResponse response, int errorCode, String error, String errorMessage)
        ...
        try {
            errorResponse.getWriter().write(errorObjectMapper.writeValueAsString(
                    new ErrorResponse(Instant.now().getEpochSecond(), errorCode, error, errorMessage)));
